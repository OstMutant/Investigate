package org.ost.investigate.jms.b2b.PubSub.Transaction;
...
public class Wholesaler implements javax.jms.MessageListener {
	...
	private void publishPriceQuotes(String dealDesc, String username,
		...
		try {
			...
			System.out.println("Puplish end...");
	...
	public static void main(String argv[]) {
		...
		try {
			...
			BufferedReader stdin = new java.io.BufferedReader(
			...
			while (true) {
				...
				if (dealDesc != null && dealDesc.length() > 0) {
					if (dealDesc.substring(0, 3).equalsIgnoreCase("END")) {
						...
						float oldPrice = Float.valueOf(temp.trim())
								.floatValue();
						temp = tokenizer.nextToken();
						float newPrice = Float.valueOf(temp.trim())
								.floatValue();
						wholesaler.publishPriceQuotes(dealDesc, "WHOLESALER",
